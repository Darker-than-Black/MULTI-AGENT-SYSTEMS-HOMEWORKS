...
const RESEARCHER_EXAMPLES = [
  ...
  "Assistant behavior:",
  "1) call `knowledge_search` for local evidence",
  "2) use `web_search` only if local evidence is insufficient or the user asks for web/current sources",
  "3) provide Markdown comparison with a Sources section and no web-only claims unless web evidence was required",
  "",
...
export const RESEARCH_AGENT_SYSTEM_PROMPT = prompt([
  ...
  section("TOOL USE POLICY", [
    ...
    "- Do not add `web_search` for stable RAG, LangChain, or LLM fundamentals when `knowledge_search` returns enough evidence.",
    "- Combine `knowledge_search` and `web_search` only when local context is useful and the request needs recent/current facts, external comparison, or concepts absent from the local knowledge base.",
...
export const PLANNER_SYSTEM_PROMPT = prompt([
  ...
  section("PLANNING RULES", [
    ...
    "- For stable RAG retrieval-strategy comparisons covered by the local knowledge base, prefer `knowledge_base` only.",
    "- Do not include `web` merely because more examples may exist online; include it when recency, external sources, or missing local coverage matter.",
