# Домашнє завдання: lesson-12 baseline (TypeScript adaptation)

Цей документ фіксує поточний TypeScript-baseline для `homework-lesson-12` перед Langfuse-оновленнями.  
Кореневий `README.md` описує вимоги домашнього завдання, а тут зібрано практичні деталі про наявну реалізацію, валідації та тестовий шар.
...
| Було (homework-lesson-8)           | Поточний стан `homework-lesson-12`              |
|------------------------------------|--------------------------------------------------|
...
| README.md (Python)               | homework-lesson-12 (реалізація)                          |
|----------------------------------|----------------------------------------------------------|
...
> **Важливо:** `npm run validate` тепер сам піднімає `Qdrant` через `docker compose`, якщо сервіс недоступний. Для `deepeval test run tests/` як і раніше потрібні валідні `.env` ключі, а знання мають бути проіндексовані (`npm run ingest`) або підготовлені через smoke suite.
