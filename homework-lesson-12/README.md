...
Ця папка містить TypeScript-baseline мультиагентної системи, з якої стартує `homework-lesson-12`.

- `npm run validate` виконує TypeScript checks, architecture invariants, deterministic supervisor smoke tests і RAG smoke suite.
- RAG smoke suite автоматично піднімає `Qdrant` через `docker compose`, якщо сервіс ще не запущений локально.
- У репозиторії ще зберігається baseline з попереднього homework; Langfuse tracing, prompt management та online evaluators потрібно додати окремим етапом.
